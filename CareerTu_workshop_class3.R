## careertu data analytics class 3 
## exploratory data analysis and data visualization 
## homework starts from row 171
setwd("~/R/Rdata")
...
# data snapshot
summary(ratings)
ratings <- ratings %>% mutate(datefield=as_datetime(timestamp)) # pipe
summary(ratings$datefield)

movie_ratings <- movies %>% inner_join(ratings,by="movieId")

most_rate<- movie_ratings %>% group_by(title) %>% summarise(avg_rating=mean(rating), cnt=n()) %>% arrange(cnt)

library(data.table)

rating_cnt_per_member <- rating_cnt_per_member %>% filter(rating_cnt>10 & rating_cnt < 500)

rating_cnt_per_member <- ratings %>% group_by(userId) %>% summarise(rating_cnt=n())

ggplot(rating_cnt_per_member, aes(x=rating_cnt))
+geom_histogram(binwidth = 20, color='red',fill='pink')
+xlab("Moving rating count")+ylab ("number of users")+ggtitle("my title")
print(pl3)
...
#### homework assignment ####
# q1: look at rating per member , visualization 
...
# suggesting to filter movies have been rated at least 10 times and fewer than 500 times
rating_cnt_per_member <- rating_cnt_per_member %>% filter(rating_cnt >=10 & rating_cnt<= 500)
...
# q3 What genres are most popular movies ?
ratings %>% group_by(movieId) %>% summarise(max_rating = max(rating))
group_by(movies, genres)


# q4 :Create plot for how the ratings change across the year for genre of Action, Roman, Sci-fi and Western 
genres_popularity %>%
  filter(year > 1930) %>%
...
#q6 what the ages for the users have rated movies?
...
#q7 optional question :Does a movie budget affect its score? 
ggplot(web_data, aes(x = score, y = budget)) +
  geom_line()

#q8 optional question:Who is the best movie director?

ggplot(web_data, aes(x = director, y = rating)) +
  geom_bar()
